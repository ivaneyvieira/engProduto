...
private fun HugeRte.configure() {
  ...
  this.configureMenubar(
    Menubar.FILE, Menubar.EDIT, Menubar.INSERT, Menubar.VIEW, Menubar.FORMAT, Menubar.TABLE, Menubar.TOOLS
  )
  this.configurePlugins(Plugin.LISTS, Plugin.LINK, Plugin.IMAGE, Plugin.TABLE, Plugin.FULLSCREEN/*, Plugin.AUTORESIZE*/)
  
  this.configure(
    "toolbar",
    "undo redo | blocks fontsize | bold italic underline | forecolor | " + "alignleft aligncenter alignright alignjustify | bullist numlist outdent indent | " + "link image table | fullscreen"
  )
  ...
  this.configure("toolbar_sticky", true) // this.configure("min_height", "300")
}

fun (@VaadinDsl HasComponents).hugeRTE(label: String = "", block: (@VaadinDsl HugeRte).() -> Unit) {
