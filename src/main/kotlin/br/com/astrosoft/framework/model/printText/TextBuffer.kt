...
class TextBuffer {
  ...
  fun printEspPos() =
    buffer.toString().replace("<B>", NEGRITO_ON).replace("</B>", NEGRITO_OFF).replace("<N>", SET_FONT_SMALL)
      .replace("</N>", "").replace("<E>", EXPANDIDO_ON).replace("</E>", EXPANDIDO_OFF).replace("<EB>", EXPANDIDON_ON)
      .replace("</EB>", EXPANDIDON_OFF)
  
  fun printHtml() = buffer.toString().removerInicializer().removerCodigoBarras().removeFinalize().replace(" ", "&nbsp;")
    .replace("<B>", "<strong><font size=${FONT_SIZE}>").replace("</B>", "</font></strong>")
    .replace("<N>", "<font size=${FONT_SIZE}>").replace("</N>", "</font>")
    .replace("<E>", "<font size=${FONT_SIZE * 2}>").replace("</E>", "</font>")
    .replace("<EB>", "<strong><font size=${FONT_SIZE * 2}>").replace("</EB>", "</font></strong>")
  ...
  private fun String.removerCodigoBarras(): String {
    ...
    return this.replace(EscPosConst.BARCODE_HEIGHT, "").replace(EscPosConst.BARCODE_WIDTH, "").replace(padrao3, "")
  }
}
