...
abstract class ReportBuild<T> {
  ...
  protected fun <V : Any> columnReport(dataType: DRIDataType<in V, V>,
    ...
    return col.column(if (oculto) "" else if (header == "") property.name else header, property.name, dataType).apply {
      ...
      if (register) { //columnsMap[property] = this
        addColumn(this)
      }
      if (oculto) {
  ...
  open fun makeReport(itens: List<T>): JasperReportBuilder {
    ...
    return report().title(titleBuider(propriedades)).setTemplate(Templates.reportTemplate).columns(*colunms).apply {
      if (grupos.isNotEmpty()) this.columnGrid(*grupos)
      else this.columnGrid(*colunms)
    }.setDataSource(itens).setPageFormat(propriedades.pageType, propriedades.pageOrientation)
      .setPageMargin(margin(propriedades.margem)).summary(pageFooterBuilder())
      .subtotalsAtSummary(*subtotalBuilder().toTypedArray())
      .setSubtotalStyle(stl.style().setPadding(2).setTopBorder(stl.pen1Point()))
      .pageFooter(cmp.pageNumber().setHorizontalTextAlignment(RIGHT).setStyle(stl.style().setFontSize(8)))
      .setColumnStyle(
        stl.style().setFontSize(propriedades.detailFonteSize).setLeftPadding(4).setRightPadding(4).setTopPadding(2)
          .setBottomPadding(2).setBottomBorder(stl.pen1Point())
      ).setDetailStyle(stl.style().setFontSize(propriedades.detailFonteSize)).apply {
