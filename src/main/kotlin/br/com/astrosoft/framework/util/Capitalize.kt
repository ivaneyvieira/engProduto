...
private val LOCALE_PT_BR: Locale =
...
fun padronizarRazaoSocial(
  razaoSocial: String,
  siglasAdicionais: Set<String> = emptySet()
): String {
...
fun produzirNomeReduzido(
  ...
  val nomePadronizado = padronizarRazaoSocial(
    ...
    siglasAdicionais = siglasAdicionais
  )
  ...
  for (palavra in nomePadronizado.split(Regex("\\s+"))) {
    ...
    val ehPalavraDeLigacao =
        palavraNormalizada in PALAVRAS_NAO_CONTABILIZADAS
...
private fun formatarPalavra(
  ...
  return palavra
    .lowercase(LOCALE_PT_BR)
    .split("-")
    .mapIndexed { indice, parte ->
      val primeiraParte =
          primeiraPalavra && indice == 0

      val parteNormalizada =
          normalizarParaComparacao(parte)

      if (
        !primeiraParte &&
        parteNormalizada in PALAVRAS_NAO_CONTABILIZADAS
      ) {
        parte
      } else {
...
private fun capitalizarPrimeiraLetra(
  ...
  return buildString {
    ...
    append(
      texto.substring(indice, indice + 1)
        .uppercase(LOCALE_PT_BR)
    )
