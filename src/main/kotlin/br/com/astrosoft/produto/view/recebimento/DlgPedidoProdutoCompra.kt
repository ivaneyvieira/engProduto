...
class DlgPedidoProdutoCompra(val viewModel: TabPedidoViewModel, var pedido: PedidoCapa) {
  ...
  fun showDialog(onClose: () -> Unit) {
    ...
    form = SubWindowForm("Produtos do Pedido $numero Loja: $loja", toolBar = {}, onClose = {
      onClose()
    }) {
  ...
  private fun HorizontalLayout.createGridProdutos() {
    gridDetail.apply {
      ...
      isMultiSort = false
      columnGrid(PedidoProdutoCompra::codigoNum, "Código")
      columnGrid(PedidoProdutoCompra::descricao, "Descrição", width = "20rem")
