...
class DlgProdutoKardexCD(val viewModel: TabControleCDViewModel, val produto: ProdutoEstoque,
  ...
  private fun HorizontalLayout.createGridProdutos() {
    gridDetail.apply {
      ...
      columnGrid(ProdutoKardex::ljDoc, "Lj Doc").right() //columnGrid(ProdutoKardex::quantDevolucao, "Garantia").right()
  ...
  fun update() {
    val kardex = viewModel.kardex(produto, dataIncial).filter {
      ...
      ) || data.startsWith(pesquisa) || user.contains(
        pesquisa, ignoreCase = true
      ) || nfEnt.toString() == pesquisa || tipo.contains(
        pesquisa, ignoreCase = true
      )
    }
