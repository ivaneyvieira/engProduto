...
fun lerRegistrosH010(caminhoArquivo: String, onAviso: ((SpedParseWarning) -> Unit)? = null): List<ArquivoSped> {
  ...
  File(caminhoArquivo).forEachLine { linha ->
    ...
    if (campos.size < 8) {
      onAviso?.invoke(
        SpedParseWarning(
          linhaNumero = linhaNumero, linha = linha, motivo = "Campos insuficientes: ${campos.size}"
        )
      )
      return@forEachLine
    }
...
private fun parseCampo(valor: String,
  ...
  if (parsed == null && valor.isNotBlank()) {
    onAviso?.invoke(
      SpedParseWarning(
        linhaNumero = linhaNumero, linha = linha, motivo = "Campo $nomeCampo invalido: '$valor'"
      )
    )
  }
