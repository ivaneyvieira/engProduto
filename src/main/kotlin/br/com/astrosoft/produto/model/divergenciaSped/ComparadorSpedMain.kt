...
private fun compararInventarios(planilha: List<PlanilhaInventario>,
                                sped: List<ArquivoSped>): List<DivergenciaInventarioDetalhe> {
    ...
    when {
      excel != null && spedItem != null -> {
        val mesmaQuantidade = valoresIguaisComTolerancia(
          excel.quantidade, spedItem.quantidade, toleranciaQuantidade
        )
        val mesmoValorUnitario = valoresIguaisComTolerancia(
          excel.valorUnitario, spedItem.valorUnitario, toleranciaValor
        )
...
private fun resumirPlanilha(planilha: List<PlanilhaInventario>): Map<String, InventarioResumo> {
  return planilha.groupBy { it.numeroProduto.trim() }.mapValues { (codigo, itens) ->
    ...
    InventarioResumo(
      codigo = codigo, descricao = descricao, quantidade = quantidade, valorUnitario = valorUnitario
    )
  }
}

private fun resumirSped(sped: List<ArquivoSped>): Map<String, InventarioResumo> {
  return sped.groupBy { it.codItem.trim() }.mapValues { (codigo, itens) ->
    val quantidade = itens.fold(BigDecimal.ZERO) { acc, item -> acc + item.quantidade }
    ...
    InventarioResumo(
      codigo = codigo, descricao = "", quantidade = quantidade, valorUnitario = valorUnitario
    )
  }
}

private fun calcularValorUnitario(quantidade: BigDecimal, valorTotal: BigDecimal, fallback: BigDecimal?): BigDecimal {
...
private fun gerarPlanilhaSaida(caminhoSaida: String, divergencias: List<DivergenciaInventarioDetalhe>) {
  ...
  val headers = listOf(
    "Codigo", "Descricao", "Quantidade Excel", "Quantidade SPED", "Valor Unit Excel", "Valor Unit SPED", "Tipo"
  )
