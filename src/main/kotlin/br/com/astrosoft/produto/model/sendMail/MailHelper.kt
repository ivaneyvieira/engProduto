...
// API pública
// ---------------------------
suspend fun sendEmailAsync(request: EmailRequest): Result<Unit> = withContext(Dispatchers.IO) {
  runCatching {
...
private fun String.isEmailSyntaxValid(): Boolean = try {
...
private fun buildMessage(session: Session, request: EmailRequest): MimeMessage {
  val msg = MimeMessage(session).apply {
    ...
    setRecipients(
      Message.RecipientType.TO, request.to.map(::InternetAddress).toTypedArray()
    )
    
    if (request.cc.isNotEmpty()) {
      setRecipients(
        Message.RecipientType.CC, request.cc.map(::InternetAddress).toTypedArray()
      )
    }
    
    if (request.bcc.isNotEmpty()) {
      setRecipients(
        Message.RecipientType.BCC, request.bcc.map(::InternetAddress).toTypedArray()
      )
    }
  ...
  val multipart = MimeMultipart("mixed").apply {
    ...
    request.anexos.forEach { anexo ->
      val attachmentPart = MimeBodyPart().apply { // Evita problemas com acentos no nome
...
// Exceções de domínio
// ---------------------------
class EmailDestinoInexistenteException(val invalidRecipients: List<String>, message: String, cause: Throwable? = null) :
    RuntimeException(message, cause)

class EmailEnvioFalhouException(message: String, cause: Throwable? = null) : RuntimeException(message, cause)
...
private fun mapToDomainException(ex: Throwable): Throwable { // 1) Servidor rejeitou destinatário (muito comum em mailbox inexistente)
  ...
  if (ex is MessagingException) {
    return EmailEnvioFalhouException(
      message = "Falha ao enviar e-mail: ${ex.message}", cause = ex
    )
  }
