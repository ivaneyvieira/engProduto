...
class EntradaDevCli(
  ...
  fun marcaImpresso(impressora: Impressora) {
    saci.marcaTrocaImpresso(
      invno = invno, storeno = storeno ?: 0, pdvno = pdvVenda ?: 0, xano = xano ?: 0, impressora = impressora
    )
    ...
    when {
      isReembolso()    -> {
        val saldoDevolucao = SaldoDevolucao(
          ...
          notaDev = NotaVendaDados(
            loja = loja, nfVenda = nfVenda ?: "", nfDev = notaFiscal ?: ""
          ),
          saldo = valor ?: 0.00
        )
      ...
      isMuda()         -> {
        ...
        val saldoDevolucao = SaldoDevolucao(
          invno = invno, custnoDev = custno, custnoMuda = mudaCliente, tipo = this.tipoObs, saldo = valor ?: 0.00
        )
  ...
  fun isReembolso(): Boolean {
    return remarks?.contains("EST CARTAO", ignoreCase = true) == true || remarks?.contains(
      "EST BOLETO", ignoreCase = true
    ) == true || remarks?.contains("REEMBOLSO", ignoreCase = true) == true || remarks?.contains(
      "GARANTIA", ignoreCase = true
    ) == true || remarks?.contains("EST DEP", ignoreCase = true) == true
  }
  ...
  fun autoriza(user: UserSaci) {
    saci.autorizaNota(
      invno = invno, storeno = storeno ?: 0, pdvno = pdvno ?: 0, xano = xano ?: 0, user = user
    )
  }
  
  fun isTipoMisto(): Boolean {
    return "TRO.* M.*".toRegex().matches(this.tipoObs) || "EST.* M.*".toRegex()
      .matches(this.tipoObs) || "REE.* M.*".toRegex().matches(this.tipoObs)
  }
  ...
  fun naoLiberado(): Boolean {
    ...
    val tipoOk =
      tipo == ESolicitacaoTroca.Estorno || tipo == ESolicitacaoTroca.MudaCliente || tipo == ESolicitacaoTroca.Reembolso || produto == EProdutoTroca.Sem || produto == EProdutoTroca.Misto
    return if (tipoOk) {
  ...
  fun desfazTroca() { //if (isNaoInformado() && impressora.isNullOrEmpty().not()) {
...
data class FiltroEntradaDevCli(val loja: Int, val query: String, val dataI: LocalDate?, val dataF: LocalDate?,
                               val dataLimiteInicial: LocalDate?, val impresso: Boolean?, val tipo: ETipoDevCli,
                               var dataCorte: LocalDate?, val cancelado: Boolean = false)
