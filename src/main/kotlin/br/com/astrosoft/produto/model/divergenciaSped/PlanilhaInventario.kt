  ...
  File(caminhoArquivo).inputStream().use { input ->
    WorkbookFactory.create(input).use { workbook ->
      ...
      for (rowIndex in 1..sheet.lastRowNum) { // comeca da linha 2, ignorando o cabecalho
        ...
        if (numeroProduto.isBlank() && descricaoProduto.isBlank() && isZero(quantidade) && isZero(custoMedio) && isZero(
            valorTotal
          )
        ) {
          continue
...
private fun validarCabecalho(sheet: Sheet, formatter: DataFormatter, cabecalhosEsperados: List<String>?) {
  ...
  if (encontradoNormalizado != esperadoNormalizado) {
    throw IllegalArgumentException(
      "Cabecalho invalido na planilha. Esperado: ${cabecalhosEsperados.joinToString()} " + "Encontrado: ${encontrados.joinToString()}"
    )
  }
}
...
private fun getBigDecimal(row: Row, cellIndex: Int, formatter: DataFormatter, evaluator: FormulaEvaluator): BigDecimal {
