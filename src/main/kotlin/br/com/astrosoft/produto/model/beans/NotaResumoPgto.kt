...
class NotaResumoPgto(var loja: Int?,
  ...
  fun grupo(filtro: FiltroNotaResumoPgto): String {
    ...
    val grupoParcela = if (agrupaParcelas) {
      if (agrupaTipoPagamento) "${(mediaPrazo ?: 0.00).format()}-$tipoPgto"
      else (mediaPrazo ?: 0.00).format()
    } else {
      if (agrupaTipoPagamento) tipoPgto ?: ""
      else "${mult.format("0.0000")}-${(mediaPrazo ?: 0.00).format()}-$tipoPgto"
    }
