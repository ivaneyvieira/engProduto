...
class TabEstoqueDadosDevProdutoViewModel(val viewModel: EstoqueCDViewModel) {
  ...
  fun updateKardex() = viewModel.exec {
    val produtos = subView.produtosSelecionados().flatMap {
...
interface ITabEstoqueDadosDevProduto : ITabView {
  ...
  fun autorizaEntrega(produtos: List<DadosDevProduto>, block: (user: UserSaci, produtos: List<DadosDevProduto>) -> Unit)
  
  fun autorizaRecebimento(produtos: List<DadosDevProduto>,
