...
SELECT DISTINCT
    H.custno,
    MAX(H.invno) AS invno
FROM sqldados.cthcr2 AS H LEFT JOIN sqldados.inv AS I
                                        USING (invno) INNER JOIN sqldados.custp AS C
                                        ON C.no = H.custno
WHERE
      invno > 0
...
FROM sqldados.nf AS N INNER JOIN sqldados.custp AS C
                                     ON C.no = N.custno INNER JOIN T_DEV AS D
...
FROM sqldados.custp AS C LEFT JOIN T_DEV AS D
                                       ON C.no = D.custno LEFT JOIN sqldados.inv AS I
                                       ON D.invno = I.invno LEFT JOIN sqldados.vend AS V
                                       ON I.vendno = V.no LEFT JOIN T_VENDA AS N
                                       ON N.invno = D.invno LEFT JOIN T_LOJ AS L
                                       ON C.no = L.custno
WHERE
      C.saldoDevolucao != 0
