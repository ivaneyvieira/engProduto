...
FROM
  sqldados.eoprd               AS E
    ...
    LEFT JOIN  sqldados.eoprdf AS O
               ON (O.storeno = E.storeno AND O.ordno = E.ordno AND O.prdno = E.prdno AND O.grade = E.grade)
    LEFT JOIN  sqldados.xaprd  AS X
               ON X.storeno = EO.storeno AND X.nfno = EO.nfno AND X.nfse = EO.nfse AND X.prdno = E.prdno AND
                  X.grade = E.grade
