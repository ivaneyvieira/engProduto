...
SELECT
    ...
    SUM(((POW(2, 1) & bits) = 0) OR (aliqPis != 165) OR (aliqCofins != 760) OR (cstPis != 1) OR (cstCofins != 1) OR
        (cstPisIn != 50) OR (cstCofinsIn != 50))    AS ctErroPisCofins,
    SUM(CASE auxStr1
            ...
            ELSE 1 END)                             AS ctErroRotulo
FROM sqldados.spedprdst INNER JOIN T_PRD_FILTER
                                       USING (prdno)
GROUP BY
    prdno;
...
FROM T_PRD_FILTER AS F LEFT JOIN sqldados.stk AS S
...
SELECT
    ...
    CASE PD.tipoGarantia WHEN 0 THEN 'Dia' WHEN 1 THEN 'Semana' WHEN 2 THEN 'Mês' WHEN 3 THEN 'Ano' ELSE '' END AS uGar,
    PD.garantia                                                                                                 AS tGar,
...
FROM T_PRD_FILTER AS PF LEFT JOIN sqldados.prd AS PD
                                      ON PF.prdno = PD.no LEFT JOIN sqldados.prdalq AS R
                                      ON R.prdno = PD.no LEFT JOIN sqldados.vend AS V
                                      ON V.no = PD.mfno LEFT JOIN sqldados.spedprd AS S
                                      ON PD.no = S.prdno LEFT JOIN T_STK AS STK
                                      ON STK.prdno = PF.prdno LEFT JOIN T_PRD_ST AS ST
...
WHERE
  ...
  AND ((:pisCofN = 'N') OR (:pisCofN = 'S' AND ctErroPisCofins > 0))
  AND ((:rotuloN = 'N') OR (:rotuloN = 'S' AND ctErroRotulo > 0))
