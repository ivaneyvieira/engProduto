...
WHERE
      (N.issuedate >= P.dataVenda)
  AND N.status <> 1
GROUP BY
    P.storeno, P.prdno, P.grade;
...
SELECT
    ...
    P.prdno                    AS prdno,
    P.grade                    AS grade,
    SUM(ROUND(-S.qtty / 1000)) AS vendas
FROM T_PRD AS P LEFT JOIN sqldados.stkmov AS S
                              USING (storeno, prdno, grade)
WHERE
      (S.date >= P.dataVenda)
  AND (S.qtty < 0)
...
UPDATE sqldados.qtd_vencimento AS Q LEFT JOIN T_VENDAS AS V USING (storeno, prdno, grade) LEFT JOIN T_STKMOV AS S USING (storeno, prdno, grade)
SET
    Q.vendas = IFNULL(V.vendas, 0) + IFNULL(S.vendas, 0)
WHERE
