...
SELECT
    ...
    ''           AS auxStr3,
    ''           AS auxStr4
FROM sqldados.spedprd AS S INNER JOIN T_PRD_ST AS T
                                          USING (prdno) INNER JOIN sqldados.prdalq AS A
                                          ON A.prdno = S.prdno INNER JOIN sqldados.prd AS P
                                          ON P.no = S.prdno, T_LOJAS AS L;

REPLACE INTO
    sqldados.spedprdst(aliqPis, aliqCofins, auxLong1, auxLong2, auxLong3, auxLong4, auxLong5, auxLong6,
...
SELECT
    ...
    IF(storeno = 8, CASE auxStr1
                        ...
                        ELSE auxStr2 END, auxStr2) AS auxStr2,
    auxStr3,
    auxStr4
FROM T_SPEDST