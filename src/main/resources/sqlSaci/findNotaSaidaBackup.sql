...
SELECT storeno AS storeno, ordno AS ordno, SUM(E.bits & POW(2, 1)) AS tipoR, SUM(NOT E.bits & POW(2, 1)) AS tipoE
FROM
...
SELECT N.storeno                          AS loja,
       ...
       CASE
         WHEN tipo = 0 AND N.nfse >= 10 THEN 'NFCE'
         WHEN tipo = 0 AND N.nfse < 10  THEN 'NFE'
         WHEN N.tipo = 1                THEN 'TRANSFERENCIA'
         WHEN N.tipo = 2                THEN 'DEVOLUCAO'
         WHEN N.tipo = 3                THEN 'SIMP_REME'
         WHEN N.tipo = 4                THEN 'ENTRE_FUT'
...
WHERE (issuedate >= :dataInicial OR :dataInicial = 0)
  ...
  AND CASE :notaEntrega
        WHEN 'S' THEN (N.storeno != :loja OR :loja = 0) AND IFNULL(tipoR, 0) = 0 AND N.tipo NOT IN (0, 1)
  ...
  AND CASE
        WHEN :marca IN (0, 999) THEN (((N.tipo = 4) AND IFNULL(tipoE, 0) > 0)/*Retira Futura*/
          ...
          OR (N.tipo = 0 AND N.nfse = 1) OR (N.tipo = 0 AND N.nfse >= 10) OR (N.tipo = 1 AND N.nfse = 5))
                                ELSE TRUE
      END
GROUP BY N.storeno, N.pdvno, N.xano, SUBSTRING_INDEX(X.c5, '-', 1), SUBSTRING_INDEX(X.c4, '-', 1)
HAVING (@PESQUISA = '' OR numero LIKE @PESQUISA_START OR notaEntrega LIKE @PESQUISA_START OR cliente = @PESQUISA_NUM OR
