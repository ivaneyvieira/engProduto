...
SELECT
    ...
    CASE
        ...
        ELSE TRIM(IFNULL(GROUP_CONCAT(DISTINCT BC.barcode SEPARATOR ','), P.barcode)) END AS barcodeStrList,
...
FROM sqldados.prd AS P
         ...
         LEFT JOIN (SELECT
                        prdno,
                        grade,
                        SUM(qtty_atacado) AS qtty_atacado,
                        SUM(qtty_varejo)  AS qtty_varejo
                    FROM sqldados.stk
                    WHERE
