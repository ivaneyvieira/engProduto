...
SELECT I.invno                                                                           AS invno,
       ...
       CAST(CONCAT(IFNULL(NF1.nfno, NF2.nfno), '/',
                   IFNULL(NF1.nfse, NF2.nfse)) AS CHAR)                                  AS nfVenda, DATE (IFNULL(NF1.issuedate, NF2.base_iss_amt)) AS nfData, IFNULL(NF1.custno, NF2.custno) AS custno, IFNULL(NF1.empno, NF2.empno) AS empno, C.name AS cliente, E.sname AS vendedor, @NOTA := TRIM (SUBSTRING_INDEX(
    TRIM (SUBSTRING_INDEX(SUBSTRING_INDEX(REPLACE(I.remarks, 'NFE', 'NF'), 'NF', 2), 'NF', -1)), ' ', 1)) AS nfRmk, SUBSTRING_INDEX(@NOTA, '/', 1) * 1 AS nfno, MID(SUBSTRING_INDEX(SUBSTRING_INDEX(@NOTA, '/', 2), '/', -1), 1, 2) AS nfse
