...
CREATE
TEMPORARY TABLE IF NOT EXISTS T_USER
(
  PRIMARY KEY (login)
)
...
CREATE
TEMPORARY TABLE IF NOT EXISTS T_NF
(
...
UPDATE sqldados.xaprd2 AS X
    ...
    USING (storeno, pdvno, xano)
    LEFT JOIN T_USER AS T
ON T.login = SUBSTRING_INDEX(X.c4, '-', 1)
...
UPDATE sqldados.xaprd2 AS X
    ...
    USING (storeno, pdvno, xano)
    LEFT JOIN T_USER AS T
ON T.login = SUBSTRING_INDEX(X.c5, '-', 1)
