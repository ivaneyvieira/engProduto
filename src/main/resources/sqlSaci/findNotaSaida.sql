...
SELECT
    ...
    P.s3                                      AS userno,
    U.login                                   AS usuario
FROM sqlpdv.pxa AS P LEFT JOIN sqldados.users AS U
                                   ON U.no = P.s3
WHERE
      P.cfo IN (5117, 6117)
...
FROM T_V AS V LEFT JOIN T_E AS E
                            USING (storeno, ordno)
GROUP BY
    V.storeno, V.pdvno, V.xano;
...
SELECT
    ...
    CASE
        ...
        WHEN N.tipo = 4 THEN IF(IFNULL(T.tipoE, 0) = 0 AND IFNULL(T.tipoR, 0) > 0,
                                IF(N.storeno != :loja AND :loja != 0 AND N.nfse = 3, '', 'SIMP_REME'), 'ENTRE_FUT')
...
FROM sqldados.nf                         AS N
         ...
         LEFT JOIN  sqldados.emp         AS M
                        ON N.s16 = M.no
WHERE
      (N.l16 >= :dataEntregaInicial OR :dataEntregaInicial = 0)
  ...
  AND ((:marca IN (0, 999) AND ((N.tipo = 4 AND IFNULL(T.tipoE, 0) > 0) -- Retira Futura
    ...
    OR (N.tipo = 0 AND (N.nfse = 1 OR N.nfse >= 10)) OR (N.tipo = 1 AND N.nfse = 5) OR
                                (IFNULL(CG.storeno, 0) != :loja) OR (N.nfse = 7))) OR :marca NOT IN (0, 999))
GROUP BY
    N.storeno, N.pdvno, N.xano;
...
SELECT
    ...
    usuarioSingExp,
    usuarioSep
FROM T_QUERY AS Q INNER JOIN sqldados.xaprd2 AS X
                                 ON X.storeno = Q.loja AND X.pdvno = Q.pdvno AND X.xano = Q.xano
WHERE
    (@PESQUISA = '' OR numero LIKE @PESQUISA_START OR notaEntrega LIKE @PESQUISA_START OR cliente = @PESQUISA_NUM OR
...
GROUP BY
    Q.loja, Q.pdvno, Q.xano
HAVING
    ((:marca = 0 AND countExp > 0) OR (:marca = 1 AND countCD > 0) OR (:marca = 2 AND countEnt > 0) OR (:marca = 999))
