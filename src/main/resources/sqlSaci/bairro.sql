select no, name FROM sqldados.route AS R;

DROP TEMPORARY TABLE IF EXISTS T_ROTAS;
CREATE TEMPORARY TABLE T_ROTAS
(
  PRIMARY KEY (routeno)
)
...
FROM sqldados.ctadd AS A
       ...
       LEFT JOIN sqldados.rotasAdd AS RA1
                 ON RA1.cidade = 'TERESINA'
                   AND RA1.bairro = IF(prefixo = 0, R.routeName, TRIM(MID(R.routeName, LOCATE(' ', R.routeName), 100)))
