...
FROM sqldados.stk AS S INNER JOIN T_PRD
                                      USING (prdno)
WHERE
      S.storeno = 1
...
FROM T_PRD AS P LEFT JOIN sqldados.prdbar AS B
                              ON P.prdno = B.prdno
GROUP BY
    prdno, gradeOpt;
...
SELECT
    ...
    ROUND(IFNULL(JS.estoque, 0)) + ROUND(IFNULL(DS.estoque, 0)) + ROUND(IFNULL(MR.estoque, 0)) +
    ROUND(IFNULL(MF.estoque, 0)) + ROUND(IFNULL(PK.estoque, 0)) +
    ROUND(IFNULL(TM.estoque, 0))                                                                               AS estoque,
    P.taxno                                                                                                    AS trib,
    P.mfno_ref                                                                                                 AS refForn,
    P.weight_g                                                                                                 AS pesoBruto,
    CASE P.tipoGarantia WHEN 0 THEN 'Dia' WHEN 1 THEN 'Semana' WHEN 2 THEN 'Mês' WHEN 3 THEN 'Ano' ELSE '' END AS uGar,
...
FROM T_BAR AS B LEFT JOIN T_STK_JS AS JS
                              USING (prdno, gradeOpt) LEFT JOIN T_STK_DS AS DS
                              USING (prdno, gradeOpt) LEFT JOIN T_STK_MR AS MR
                              USING (prdno, gradeOpt) LEFT JOIN T_STK_MF AS MF
                              USING (prdno, gradeOpt) LEFT JOIN T_STK_PK AS PK
                              USING (prdno, gradeOpt) LEFT JOIN T_STK_TM AS TM
                              USING (prdno, gradeOpt) INNER JOIN T_PRD AS P
                              USING (prdno) LEFT JOIN sqldados.prdalq AS R
                              USING (prdno) LEFT JOIN sqldados.prd2 AS P2
                              ON P.prdno = P2.prdno LEFT JOIN sqldados.vend AS V
                              ON V.no = P.mfno LEFT JOIN sqldados.spedprd AS N
                              ON N.prdno = P.prdno LEFT JOIN T_LOC AS L
...
FROM T_PRD AS R INNER JOIN sqldados.vendaDate AS X
                               ON X.prdno = R.prdno
WHERE
...
SELECT
    ...
    MAX(date)                        AS date,
    SUM(quant / 1000)                AS qtty
FROM T_PRD AS R INNER JOIN sqldados.compraDate P
                               ON P.prdno = R.prdno
WHERE
...
FROM sqldados.iprdAdicional AS A INNER JOIN T_PRD AS P
                                                USING (prdno) INNER JOIN sqldados.iprd AS I
                                                USING (invno, prdno, grade) INNER JOIN sqldados.inv AS N
                                                USING (invno) LEFT JOIN sqldados.validadeAdicional AS V
