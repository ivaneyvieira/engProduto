...
SELECT
    ...
    qtty,
    0 AS marca
FROM sqldados.invnfe AS N INNER JOIN sqldados.iprd AS X
                                         USING (invno) INNER JOIN sqldados.invConferencia AS C
                                         USING (nfekey)
WHERE
    nfekey != '';
...
FROM T_MESTRE LEFT JOIN T_IPRD_S                                            AS S
                            USING (nfekey, prdno, grade) LEFT JOIN T_IPRD_C AS C
                            USING (nfekey, prdno, grade);

SELECT
    IFNULL(NI.invno, 0)                               AS ni,
    ...
    I.nfekey                                          AS chave,
    I.marca                                           AS marca
FROM sqldados.invConferencia AS I LEFT JOIN T_IPRD_M AS P
                                                USING (nfekey) LEFT JOIN sqldados.invnfe AS N
                                                ON N.nfekey = I.nfekey LEFT JOIN sqldados.inv AS NI
                                                USING (invno) LEFT JOIN sqldados.vend AS V
                                                ON V.no = NI.vendno
WHERE
