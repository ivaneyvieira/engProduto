...
SET
    O.auxMy2 = T.qtty, T.auxMy2 = T.qtty
WHERE
    O.auxMy2 = 0;
...
SELECT
    prdno,
    grade,
    GROUP_CONCAT(mesAno) AS vencimentoStrList
...
SELECT
    ...
    IF(IFNULL(X.grade, '') = '', CONCAT(TRIM(P.barcode), ',', GROUP_CONCAT(TRIM(IFNULL(B.barcode, '')) SEPARATOR ',')),
