...
FROM
  sqldados.prd                 AS P
    ...
    INNER JOIN sqldados.store  AS S
               ON S.no IN (2, 3, 4, 5, 8) AND (S.no = :loja OR :loja = 0)
WHERE (TRIM(P.no) = @CODIGO OR @CODIGO = '')
  ...
  AND CASE :caracter
        ...
        WHEN 'T' THEN TRUE
                 ELSE FALSE
      END
  AND (@PESQUISA = '' OR P.name LIKE @PESQUISALIKE OR P.mfno = @PESQUISANUM)
...
SELECT storeno, prdno, grade, ROUND(SUM(qtty_varejo + stk.qtty_atacado) / 1000) AS estoqueLoja
FROM
  sqldados.stk
    INNER JOIN T_PRD
...
WHERE (:ano = 0 OR MID(vencimento, 1, 4) = :ano)
  ...
  AND (@PESQUISA = '' OR descricao LIKE @PESQUISALIKE OR vendno = @PESQUISANUM OR unidade = @PESQUISA)
