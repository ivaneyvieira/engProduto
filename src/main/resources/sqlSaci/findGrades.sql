...
WHERE (P.no = LPAD(:codigo, 16, ' ') OR
       TRIM(IF(B.grade IS NULL, IFNULL(IF(LENGTH(TRIM(P.barcode)) = 13, P.barcode, NULL), P2.gtin), B.barcode)) =
       :codigo)
...
FROM
  T_PRD                    AS P
    LEFT JOIN sqldados.stk AS S
              ON S.prdno = P.prdno AND S.grade = P.grade AND S.storeno = :loja
