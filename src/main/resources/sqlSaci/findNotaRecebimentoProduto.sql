...
SELECT
    prdno,
    grade,
    GROUP_CONCAT(DISTINCT TRIM(barcode)) AS barcodeList
...
SELECT
    ...
    CASE
        ...
        ELSE '' END     AS tipoNota,
    selecionado         AS selecionado
FROM sqldados.iprd AS I INNER JOIN sqldados.inv AS N
                                       USING (invno) INNER JOIN sqldados.store AS L
                                       ON L.no = N.storeno LEFT JOIN T_NOTA_FILE AS F
                                       ON F.invno = I.invno LEFT JOIN sqldados.iprdAdicional AS A
...
SELECT
    prdno,
    grade,
    SUM((qtty_atacado + qtty_varejo) / 1000) AS estoque
FROM sqldados.stk AS S INNER JOIN (SELECT prdno, grade FROM T_NOTA GROUP BY prdno, grade) AS N
                                      USING (prdno, grade)
WHERE
    (storeno = :loja OR :loja = 0)
GROUP BY
    prdno, grade;
...
SELECT
    ...
    selecionado                                                                                                   AS selecionado
FROM T_NOTA AS N LEFT JOIN sqldados.users AS ER
                               ON ER.no = N.usernoRecebe LEFT JOIN sqldados.vend AS V
                               ON V.no = N.vendno LEFT JOIN sqldados.custp AS C
                               ON C.cpf_cgc = V.cgc INNER JOIN sqldados.prd AS P
                               ON P.no = N.prdno LEFT JOIN T_BARCODE AS B
                               ON B.prdno = N.prdno AND B.grade = N.grade LEFT JOIN T_LOC AS L
                               ON L.prdno = N.prdno AND L.grade = N.grade LEFT JOIN sqldados.prdloc AS LS
                               ON LS.prdno = N.prdno AND LS.grade = N.grade AND LS.storeno = 4 LEFT JOIN T_EST AS E
...
FROM T_QUERY
WHERE
      (@PESQUISA = '' OR ni = @PESQUISA_NUM OR nfEntrada LIKE @PESQUISA_LIKE OR custno = @PESQUISA_NUM OR
