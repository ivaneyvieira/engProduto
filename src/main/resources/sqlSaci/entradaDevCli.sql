...
SELECT
    ...
    CASE
        ...
        ELSE '' END           AS obsNI
FROM sqldados.nf AS N INNER JOIN sqldados.custp AS C
                                     ON C.no = N.custno
WHERE
...
SELECT
    ...
    MID(SUBSTRING_INDEX(SUBSTRING_INDEX(@NOTA, '/', 2), '/', -1), 1,
        2)                                                                                                       AS nfse,
    I.c9                                                                                                         AS impressora,
    U.pdv                                                                                                        AS pdvVenda,
...
WHERE
  ...
  AND CASE :impresso WHEN 'S' THEN I.c9 != '' WHEN 'N' THEN I.c9 = '' WHEN 'T' THEN TRUE ELSE FALSE END;
...
SELECT
    ...
    I.notaFiscal,
    I.data,
    IF(LOCATE('/', impressora) > 0, SUBSTRING_INDEX(impressora, '/', -1), TIME_FORMAT(CURRENT_TIME, '%H:%i')) AS hora,
    I.vendno,
    I.fornecedor,
    I.custnoDev,
    I.clienteDev,
    ...
    IF(IF(I.estorno = 'N', pdvVenda, N.remarks LIKE '') IS NULL, 'N',
       'S')                                                                                                   AS fezTroca,
...
FROM T_NOTA                                AS I
         ...
         LEFT JOIN sqldados.users          AS UA
                       ON UA.no = A.userno
WHERE
      (@PESQUISA = '' OR I.invno = @PESQUISANUM OR I.loja = @PESQUISANUM OR I.notaFiscal LIKE @PESQUISASTART OR
