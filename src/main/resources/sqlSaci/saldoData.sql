...
DO
@DATA_INICIO_MES := DATE_FORMAT(@DATA_INICIAL, '%Y%m01') * 1;
DO
@DIA_ANTES := SUBDATE(@DATA_INICIAL, INTERVAL 1 DAY) * 1;
DO
@DIA_MES_ANTERIOR := LAST_DAY(SUBDATE(@DATA_INICIAL, INTERVAL 1 MONTH));
DO
@MES_ANTEIROR := DATE_FORMAT(@DIA_MES_ANTERIOR, '%Y%m');

DROP TABLE IF EXISTS Lojas;
CREATE
TEMPORARY TABLE Lojas
(
  PRIMARY KEY (storeno)
)
...
SELECT P.storeno,
       ...
       P.storeno AS loja,
       prdno,
       grade, date, CONCAT(P.nfno, '/', P.nfse) AS doc, (-(price) * 100 * qtty) AS cost, 'NF Saida' AS tipo, (-qtty * 1000 / 1000) AS quant
...
WHERE N.status <> 1
  AND date BETWEEN @DATA_INICIO_MES
  AND @DIA_ANTES
  AND N.cfo NOT IN (5922
    , 6922)
  AND (prdno = @PRDNO
   OR @PRDNO = '')
  AND (grade = @GRADE
...
FROM sqldados.xaprd AS P
         ...
         INNER JOIN sqldados.nf AS N
                    ON P.storeno = N.storeno AND P.pdvno = N.pdvno AND P.xano = N.xano
WHERE date BETWEEN @DATA_INICIO_MES
  AND @DIA_ANTES
  AND (prdno = @PRDNO
   OR @PRDNO = '')
  AND (grade = @GRADE
...
CREATE
TEMPORARY TABLE MovManual
SELECT storeno AS loja,
       prdno,
       grade, date, 'Mov Manual' AS tipo, SUM (qtty / 1000) AS quant
...
DROP
TEMPORARY TABLE IF EXISTS NFFutura;
CREATE
TEMPORARY TABLE NFFutura
(
...
CREATE
TEMPORARY TABLE NFEntrada
SELECT I.storeno AS loja,
       P.prdno,
       P.grade,
       I.date AS date,
       'NF Entrada'     AS tipo,
       SUM(qtty / 1000) AS quant
...
WHERE I.bits & POW(2
    , 4) = 0
  AND I.auxShort13 & POW(2
    , 15) = 0
  AND I.date BETWEEN @DATA_INICIO_MES
  AND @DIA_ANTES
  AND (P.prdno = @PRDNO
   OR @PRDNO = '')
  AND (grade = @GRADE
