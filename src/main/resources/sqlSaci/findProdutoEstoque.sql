...
SELECT
    prdno,
    grade,
    GROUP_CONCAT(DISTINCT localizacao ORDER BY 1) AS locSaci
...
FROM sqldados.stk AS E INNER JOIN sqldados.store AS S
                                      ON E.storeno = S.no INNER JOIN sqldados.prd AS P
                                      ON E.prdno = P.no LEFT JOIN sqldados.vend AS V
                                      ON V.no = P.mfno LEFT JOIN T_LOC_APP AS A
                                      USING (prdno, grade) LEFT JOIN T_LOC_SACI AS L1
                                      USING (prdno, grade)

WHERE
      (((P.dereg & POW(2, 2) = 0) AND (:inativo = 'N')) OR ((P.dereg & POW(2, 2) != 0) AND (:inativo = 'S')) OR
       (:inativo = 'T'))
...
FROM temp_pesquisa
WHERE
      (@PESQUISA = '' OR locSaci LIKE @PESQUISALIKE OR codigo = @PESQUISANUM OR locSaci LIKE @PESQUISALIKE OR
