...
SELECT
    ...
    IF(IFNULL(emp.sname, '') = '', NULL, TRUNCATE(CONCAT(RIGHT(inv2.c3, 4), MID(inv2.c3, 4, 2), LEFT(inv2.c3, 2)), 0) *
                                         1)                                                                    AS dataRecbedor,
...
FROM sqldados.inv2 LEFT JOIN sqldados.vend
                                 ON (vend.no = inv2.vendno) LEFT JOIN sqldados.carr AS carr
                                 ON (carr.no = inv2.carrno) LEFT JOIN sqldados.inv  AS inv
                                 ON (inv.vendno = inv2.vendno AND inv.nfname = inv2.nfname AND
                                     inv.ordno = inv2.ordno AND inv.grossamt = inv2.grossamt) LEFT JOIN sqldados.emp
                                 ON (emp.no = inv2.auxStr6 AND emp.no <> 0) LEFT JOIN sqldados.ords
...
WHERE
    loja <> 0
HAVING
    (:filtro = '' OR DATE_FORMAT(data, '%d/%m/%Y') LIKE @FILTRO_LIKE OR
