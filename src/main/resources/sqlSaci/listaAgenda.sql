...
SELECT inv2.storeno                                                                    AS loja,
  ...
  /*
  CAST(IF(TRUNCATE(CONCAT(RIGHT(inv2.c1, 4), MID(inv2.c1, 4, 2), LEFT(inv2.c1, 2)), 0) * 1 = 0, NULL,
          TRUNCATE(CONCAT(RIGHT(inv2.c1, 4), MID(inv2.c1, 4, 2), LEFT(inv2.c1, 2)), 0) * 1) AS date) AS datao,
  */
       IF(inv2.c1 <> '', TRUNCATE(CONCAT(RIGHT(inv2.c1, 4), MID(inv2.c1, 4, 2), LEFT(inv2.c1, 2)), 0) * 1,
          IF(vend.state = 'PI', DATE_ADD(inv2.issue_date, INTERVAL 2 DAY) * 1,
             DATE_ADD(inv2.issue_date, INTERVAL 7 DAY)) * 1)                           AS datao,
       ...
       CAST(IF(IFNULL(emissaoConhecimento, 0) = 0, NULL, emissaoConhecimento) AS DATE) AS emissaoConhecimento,
       inv2.invno                                                                      AS invno,
       inv2.vendno                                                                     AS forn,
       ...
       IF(IFNULL(A.data, 0) <> 0 AND IFNULL(A.hora, 0) <> 0, 'S', 'N')                 AS agendado,
