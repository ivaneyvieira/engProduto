...
FROM sqldados.nf AS N INNER JOIN sqldados.xaprd2 AS X
...
WHERE
  ...
  AND N.status <> 1
  AND ((X.prdno = :prdno OR :prdno = '') AND ((X.grade = :grade) OR :grade = ''))
GROUP BY
    storeno, ordno, numero, prdno, grade;

INSERT IGNORE
    sqldados.oprdRessu(ordno, mult, ipi, freight, icms, auxLong1, auxLong2, auxMy1, auxMy2, icmsSubst,
...
INSERT IGNORE
    sqldados.ordsRessu(no, date, vendno, discount, amt, package, custo_fin, others, eord_ordno,
...
SELECT
    ...
    SUM(SUBSTRING_INDEX(X.obs, ':', 1) != '' OR SUBSTRING_INDEX(SUBSTRING_INDEX(X.obs, ':', 2), ':', -1) != '' OR
        X.auxLong1 != NF.qtty OR X.auxLong2 != 0) AS countCor,
    SUM(NF.ordno IS NOT NULL)                     AS countNot,
    SUM(X.auxShort4 = 0 AND X.auxShort3 != 0)     AS countSelCD,
    SUM(X.auxShort4 = 1 AND X.auxShort3 != 0)     AS countSelENT,
    SUM(X.auxShort4 = 2 AND X.auxShort3 != 0)     AS countSelREC
FROM sqldados.ords                 AS N
         INNER JOIN sqldados.oprd  AS X
                        ON N.storeno = X.storeno AND N.no = X.ordno
         LEFT JOIN  T_PEDIDO_NOTA  AS NF
                        ON X.ordno = NF.ordno AND X.storeno = NF.storeno AND X.prdno = NF.prdno AND X.grade = NF.grade
...
WHERE
  ...
  AND N.no >= 100000000
  AND N.date >= @DATA
  AND ((X.prdno = :prdno AND X.auxShort4 = :marca) OR :prdno = '')
  AND ((X.grade = :grade AND X.auxShort4 = :marca) OR :grade = '')
GROUP BY
    N.storeno, N.no, L.localizacao, notaBaixa;
...
SELECT
    ...
    data,
    comprador,
    GROUP_CONCAT(DISTINCT IF(D.localizacao = '', NULL, D.localizacao) SEPARATOR ',')     AS localizacoes,
    :marca                                                                               AS marca,
    :temNota                                                                             AS temNota,
    loja                                                                                 AS loja,
    GROUP_CONCAT(DISTINCT IF(notaBaixaPedido = '', NULL, notabaixaPedido) SEPARATOR ',') AS notaBaixa,
...
FROM T_PEDIDO AS D LEFT JOIN T_OBS AS A
                                 ON A.storeno = 1 AND A.ordno = D.numero
WHERE
      (@PESQUISA = '' OR numero LIKE @PESQUISA_START OR fornecedor LIKE @PESQUISA_NUM OR comprador LIKE @PESQUISA_NUM OR
       D.localizacao LIKE @PESQUISA OR entregueNo LIKE @PESQUISA_NUM OR entreguePor LIKE @PESQUISA_LIKE OR
       entregueSPor LIKE @PESQUISA_LIKE OR notaBaixaPedido LIKE @PESQUISA_START OR transportadoNo LIKE @PESQUISA_NUM OR
       transportadoPor LIKE @PESQUISA_LIKE OR transportadoSPor LIKE @PESQUISA_LIKE OR recebidoNo LIKE @PESQUISA_NUM OR
       recebidoPor LIKE @PESQUISA_LIKE OR recebidoSPor LIKE @PESQUISA_LIKE OR usuarioNo LIKE @PESQUISA_NUM OR
